...
def spectra_generation(i):
    try:
        ...
        params[6] = params_boost['A_bary'][i]
...
np.savez('outputs/linear_matter', modes = k, features = P_lin)
np.savez('outputs/non_linear_boost', modes = k, features = nl_boost)

np.savez('outputs/log10_linear_matter', modes = k, features = np.log10(P_lin))
np.savez('outputs/log10_non_linear_boost', modes = k, features = np.log10(nl_boost))
...
np.savez('outputs/linear_matter_test', modes = k, features = P_lin)
np.savez('outputs/non_linear_boost_test', modes = k, features = nl_boost)
